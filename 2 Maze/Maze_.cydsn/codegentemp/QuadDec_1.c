...
void QuadDec_1_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        QuadDec_1_CONTROL_REG = QuadDec_1_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        QuadDec_1_TRIG_CONTROL1_REG  = QuadDec_1_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        QuadDec_1_SetInterruptMode(QuadDec_1_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        QuadDec_1_SetCounterMode(QuadDec_1_COUNT_DOWN);
        QuadDec_1_WritePeriod(QuadDec_1_QUAD_PERIOD_INIT_VALUE);
        QuadDec_1_WriteCounter(QuadDec_1_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (QuadDec_1__QUAD == QuadDec_1_CONFIG) */

    #if (QuadDec_1__TIMER == QuadDec_1_CONFIG)
        QuadDec_1_CONTROL_REG = QuadDec_1_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        QuadDec_1_TRIG_CONTROL1_REG  = QuadDec_1_TIMER_SIGNALS_MODES;
        ...
        #if (QuadDec_1_CY_TCPWM_V2 && QuadDec_1_TIMER_UPDOWN_CNT_USED && !QuadDec_1_CY_TCPWM_4000)
            QuadDec_1_WriteCounter(1u);
        #elif(QuadDec_1__COUNT_DOWN == QuadDec_1_TC_COUNTER_MODE)
            QuadDec_1_WriteCounter(QuadDec_1_TC_PERIOD_VALUE);
        #else
            QuadDec_1_WriteCounter(0u);
        #endif /* (QuadDec_1_CY_TCPWM_V2 && QuadDec_1_TIMER_UPDOWN_CNT_USED && !QuadDec_1_CY_TCPWM_4000) */
    #endif  /* (QuadDec_1__TIMER == QuadDec_1_CONFIG) */

    #if (QuadDec_1__PWM_SEL == QuadDec_1_CONFIG)
        QuadDec_1_CONTROL_REG = QuadDec_1_CTRL_PWM_BASE_CONFIG;

        #if (QuadDec_1__PWM_PR == QuadDec_1_PWM_MODE)
            QuadDec_1_CONTROL_REG |= QuadDec_1_CTRL_PWM_RUN_MODE;
            QuadDec_1_WriteCounter(QuadDec_1_PWM_PR_INIT_VALUE);
        #else
            QuadDec_1_CONTROL_REG |= QuadDec_1_CTRL_PWM_ALIGN | QuadDec_1_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (QuadDec_1_CY_TCPWM_V2 && QuadDec_1_PWM_UPDOWN_CNT_USED && !QuadDec_1_CY_TCPWM_4000)
                QuadDec_1_WriteCounter(1u);
            #elif (QuadDec_1__RIGHT == QuadDec_1_PWM_ALIGN)
                QuadDec_1_WriteCounter(QuadDec_1_PWM_PERIOD_VALUE);
            #else 
                QuadDec_1_WriteCounter(0u);
            #endif  /* (QuadDec_1_CY_TCPWM_V2 && QuadDec_1_PWM_UPDOWN_CNT_USED && !QuadDec_1_CY_TCPWM_4000) */
        #endif  /* (QuadDec_1__PWM_PR == QuadDec_1_PWM_MODE) */

        #if (QuadDec_1__PWM_DT == QuadDec_1_PWM_MODE)
            QuadDec_1_CONTROL_REG |= QuadDec_1_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (QuadDec_1__PWM_DT == QuadDec_1_PWM_MODE) */

        #if (QuadDec_1__PWM == QuadDec_1_PWM_MODE)
            QuadDec_1_CONTROL_REG |= QuadDec_1_CTRL_PWM_PRESCALER;
        #endif  /* (QuadDec_1__PWM == QuadDec_1_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        QuadDec_1_TRIG_CONTROL1_REG  = QuadDec_1_PWM_SIGNALS_MODES;
...
{
    #if (QuadDec_1_CY_TCPWM_4000)
        uint32 currentMode;
    #endif /* (QuadDec_1_CY_TCPWM_4000) */

    #if (QuadDec_1_CY_TCPWM_4000)
        currentMode = ((QuadDec_1_CONTROL_REG & QuadDec_1_UPDOWN_MASK) >> QuadDec_1_UPDOWN_SHIFT);

        if (((uint32)QuadDec_1__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)QuadDec_1__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 QuadDec_1_ReadCompare(void)
{
        ...
        regVal = QuadDec_1_COMP_CAP_REG;
        
        if (((uint32)QuadDec_1__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)QuadDec_1__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & QuadDec_1_16BIT_MASK);
    #else
...
void QuadDec_1_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)QuadDec_1__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)QuadDec_1__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
uint32 QuadDec_1_ReadCompareBuf(void)
{
        ...
        regVal = QuadDec_1_COMP_CAP_BUF_REG;
